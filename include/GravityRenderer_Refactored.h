 ...
 * @brief Refactored renderer following SOLID principles - focuses only on gravity visualization
 ...
 * @brief Focused renderer for gravity simulation visualization following Single Responsibility Principle
 * 
 * This renderer is responsible ONLY for rendering the gravity field visualization.
 * Camera management is delegated to CameraController.
 * UI rendering is delegated to UIRenderer.
...
public:
    ...
    /**
     * @brief Renders the gravity grid and bodies using injected camera and UI components
     * @param cameraController Camera system for view transformation
     * @param uiRenderer UI system for overlays (optional)
     */
    void render(const CameraController& cameraController, UIRenderer* uiRenderer = nullptr);

    /**
     * @brief Legacy render method for IRenderer interface compatibility
     */
    ...
    /**
     * @brief Updates viewport dimensions
...
private:
    ...
    /**
     * @brief Sets up 3D perspective projection
     */
     ...
     */
    Vec2 forceToColor(float forceMagnitude) const;
